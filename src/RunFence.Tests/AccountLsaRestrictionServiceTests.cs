...
public class AccountLsaRestrictionServiceTests
{
    private readonly Mock<ILoggingService> _log = new();
    private readonly Mock<IAccountValidationService> _accountValidation = new();
    private readonly AccountRestrictionHelper _helper;
    ...
    public AccountLsaRestrictionServiceTests()
    {
        ...
        var loginService = new AccountLoginRestrictionService(new GroupPolicyScriptHelper(new LogonScriptIniManager(), _log.Object), _log.Object, _accountValidation.Object);
        var lsaService = new AccountLsaRestrictionService(_log.Object, _accountValidation.Object, lsa.Object);
        _helper = new AccountRestrictionHelper(loginService, lsaService);
    }
    ...
    {
        var result = _helper.IsAccountHidden("NonExistentTestUser_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var result = _helper.IsLoginBlockedBySid("S-1-5-21-0-0-0-99999");
    ...
    public void IsLocalOnlyBySid_InvalidSid_ReturnsFalse()
    {
        ...
        var result = _helper.IsLocalOnlyBySid("S-1-5-21-0-0-0-99999");
    ...
    {
        var result = _helper.IsNoBgAutostartBySid("S-1-5-21-0-0-0-99999");
    ...
    public void SetLoginBlockedBySid_UnblockNonexistent_DoesNotThrow()
    {
        ...
        // Use an injected temp dir so no real Group Policy directories are touched.
        ...
        lsa.Setup(x => x.EnumerateAccountRights(It.IsAny<byte[]>())).Returns([]);
        var loginService = new AccountLoginRestrictionService(new GroupPolicyScriptHelper(new LogonScriptIniManager(), _log.Object, systemDir: tempDir.Path), _log.Object,
            _accountValidation.Object);
        var lsaService = new AccountLsaRestrictionService(_log.Object, _accountValidation.Object, lsa.Object);
        var helper = new AccountRestrictionHelper(loginService, lsaService);

        // The GP ini/wrapper files don't exist for this fake SID — should be a silent no-op.
        var ex = Record.Exception(() =>
            helper.SetLoginBlockedBySid("S-1-5-21-9999-9999-9999-9999", "fakeUser", blocked: false));
        Assert.Null(ex);
    }
    ...
    public void GetNoBgAutostartState_InvalidSid_ReturnsFalse()
    {
        ...
        var result = _helper.GetNoBgAutostartState("S-1-5-21-0-0-0-99999");
        Assert.False(result);
    }
    ...
    {
        var result = _helper.GetLocalOnlyState("S-1-5-21-0-0-0-99999");
        Assert.False(result);
    }
